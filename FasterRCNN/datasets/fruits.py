...
class Dataset:
    ...
    def __init__( 
        ...
        compute_feature_map_shape_fn : Callable[[int, int, int], Tuple[int, int, int]]
    ...
    def _get_filepaths( self ):
        ...
        def _split_class( images, train_ratio=0.7, val_ratio=0.1 ):
            n_total = len( images )
        ...
        apple_images = glob.glob( os.path.join( self._dir, "apple_*.jpg" ) )
        banana_images = glob.glob( os.path.join( self._dir, "banana_*.jpg" ) )
        orange_images = glob.glob( os.path.join( self._dir, "orange_*.jpg" ) )
        mixed_images = glob.glob( os.path.join( self._dir, "mixed_*.jpg" ) )
        
        apple_split  = _split_class( sorted( apple_images ) )
        banana_split = _split_class( sorted( banana_images ) )
        orange_split = _split_class( sorted( orange_images ) )
        mixed_split = _split_class( sorted( mixed_images ) )

        if self.split == "train":
            return apple_split[ "train" ] + banana_split[ "train" ] + orange_split[ "train" ] + mixed_split[ "train" ]
        elif self.split == "val":
            return apple_split[ "val" ] + banana_split[ "val" ] + orange_split[ "val" ] + mixed_split[ "val" ]
        else: # self.split == "test"
            return apple_split[ "test" ] + banana_split[ "test" ] + orange_split[ "test" ] + mixed_split[ "test" ]
    ...
    def _get_ground_truth_boxes( self, filepaths: list[ str ], allow_difficult: bool ):
        ...
        for filepath in filepaths:
            ...
            for obj in root.findall( "object" ):
                ...
                y_max = int( float( y_max ) ) - 1 # convert to 0-based pixel coordinates
