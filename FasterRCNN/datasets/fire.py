...
class Dataset:
    ...
    def _get_filepaths( self ):
        split_dir = os.path.join( self._dir, self.split )
        
        import glob
        return sorted( glob.glob( os.path.join( split_dir, "*.jpg" ) ) )
    ...
    def _get_ground_truth_boxes( self, filepaths: list[ str ], allow_difficult: bool ):
        ...
        for filepath in filepaths:
            # Identify XML file in same directory as image
            basename = os.path.splitext( os.path.basename( filepath ) )[ 0 ]
            xml_dir = os.path.dirname( filepath )
            annotation_file = os.path.join( xml_dir, basename ) + ".xml"
